...
function ENT:Initialize()
	...
	self:SetModel( "models/rtbmodels/pokemon/slowpoke.mdl" )
    ...
    //Vectors
    local min=Vector(0-(w/2),0-(l/2),0-(h/2) + 50)
    local max=Vector(w/2,l/2,h/2 + 50)
...
function ENT:SetupDataTables()
	...
	self:SetVar("HP", 103)
	 self:SetVar("ATK", 58)
	 self:SetVar("SPATK", 34)
	 self:SetVar("SPDEF", 38)
	 self:SetVar("SPD", 23)
	 
	 self:SetHealth(self:GetVar("HP"))
...
function ENT:FindEnemy()
	...
	for k, v in pairs( _ents ) do
		if ( v:IsPlayer() and v:GetModel() == "models/rtbmodels/pokemon/bulbasaur.mdl" or v:GetModel() == "models/rtbmodels/pokemon/squirtle.mdl" or v:GetModel() == "models/rtbmodels/pokemon/charmander.mdl") then
...
function ENT:RunBehaviour()
	...
	while ( true ) do
		...
		if ( self:HaveEnemy() ) then
			...
			self.loco:SetDesiredSpeed( 30 )		-- Walk speed
...
if (self:GetPos():Distance(self:GetEnemy():GetPos()) < 50) then 
		...
		self:GetEnemy():SetHealth(self:GetEnemy():Health() - 1) end					-- Since we are following the player we have to constantly remake the path
