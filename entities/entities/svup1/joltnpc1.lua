...
function ENT:Initialize()
	...
	self:SetModel( "models/rtbmodels/pokemon/jolteon.mdl" )
...
function ENT:SetupDataTables()
	...
	self:SetVar( "LVL", 1  )
	self:SetVar("DEF", self:GetVar("LVL")*1.7)
		self:SetVar("HP", 10)
	 self:SetVar("ATK", 10)
	 self:SetVar("SPATK", 10)
	 self:SetVar("SPDEF", 10)
	 self:SetVar("SPD", 10)
...
local _ents = ents.FindInSphere( self:GetPos(), self.SearchRadius )
for k, v in pairs( _ents ) do
		if ( v:IsPlayer() and v:GetModel() == "models/player/red.mdl") then
		self:SetEnemy(nil)
			return true
		end
	end
